...
exports.run = async (msg, args) => {
    if (args.length === 0) {
        ...
        if (playerRole) {
            ...
            send(msg.channel, 'There is no player role currently set. Set one using `setmain [role name]`.');
        }
    } else {
        ...
        await bot.db.set('mafia', 'main', main);
        await bot.db.set('mafia', 'phase', 'setup');
        await send(msg.channel, `**${role.name}** has been set as the player role, and the main server and channel have been established.`);
    }
};
