...
exports.run = async (msg, args) => {
    if (args.length === 0) {
        const playerRole = await bot.db.get('mafia', 'mason.role');

        if (playerRole) {
            const role = findRole(msg.guild, playerRole);
            send(msg.channel, `The current player role is ${role.name}.`);
        } else {
            send(msg.channel, 'There is no player role currently set. Set one using `setmason [role name]`.');
        }
    } else {
        ...
        await bot.db.set('mafia', 'mason', mason);
        await send(msg.channel, `**${role.name}** has been set as the mason role, and the mason server and channel have been established.`);
    }
};
